...
export class PRPCClient {
  ...
  private async getMockPNodes(filters: PNodeFilters = {}): Promise<PNode[]> {
    ...
    return filteredNodes.map(node => ({
      ...
      lat: 0,
      lng: 0,
      storageUsed: BigInt(Math.floor(node.storageUsed)),
      storageCapacity: BigInt(Math.floor(node.storageCapacity)),
      lastSeen: new Date(Date.now() - Math.random() * 3600000),
      performance: this.calculatePerformance(node),
      stake: node.stake,
      riskScore: this.calculateRiskScore(node),
      xdnScore: calculateXDNScore(node.stake, node.uptime, node.latency, this.calculateRiskScore(node)),
      registered: true,
      ...
      memoryUsed: BigInt(Math.floor(node.memoryUsed)),
      memoryTotal: BigInt(Math.floor(node.memoryTotal)),
